...
framework_excludes() {
    local fw="$1"
    local common=(--exclude=".git/" --exclude=".github/" --exclude=".env" --exclude="logs/" --exclude=".devsync.conf")
    case "$fw" in
        fastapi)
        ...
        django)
            ...
            ;;
        laravel)
            common+=(--exclude="vendor/" --exclude="node_modules/" --exclude="storage/logs/" --exclude="storage/framework/cache/" --exclude="storage/framework/sessions/" --exclude="storage/framework/views/" --exclude="bootstrap/cache/")
            ;;
        yii2)
            common+=(--exclude="vendor/" --exclude="node_modules/" --exclude="runtime/")
            ;;
    esac
    printf '%s\n' "${common[@]}"
}
...
do_sync() {
    ...
    case "$SYNC_MODE" in
        dry)
            ...
            echo ""
            rsync -avhn --delete "${EXCLUDES[@]}" "$SOURCE/" "$DEST/" --itemize-changes | grep -v '^\.' || true
            ;;
        diff)
            ...
            for e in "${EXCLUDES[@]}"; do
                diff_excludes+=(--exclude="${e#--exclude=}")
            done
